...
## ✅ Неделя 1: Инфраструктура — ВЫПОЛНЕНО
...
1. ✅ Создана структура папок:
   - `states/common/`, `states/workshops/marathon/`, `states/feed/`, `states/utilities/`
   - `core/`
   - `i18n/`
...
2. ✅ Созданы базовые файлы:
   ...
   - `config/transitions.yaml` — таблица переходов
...
4. ✅ Добавлен переключатель `USE_STATE_MACHINE` в `bot.py`

---

## ✅ Неделя 2: Marathon стейты — ВЫПОЛНЕНО
...
2. ✅ Добавлены переходы в `transitions.yaml`

3. ✅ Обновлён `states/registry.py`

---

## ✅ Неделя 3: Консультация и глобальные команды — ВЫПОЛНЕНО

**Цель:** Реализовать процесс консультации и систему глобальных переходов.

**Выполнено:**

1. ✅ Создан `states/common/consultation.py`

2. ✅ Настроены глобальные события в `transitions.yaml`

3. ✅ Переход `?` → common.consultation → _previous работает

---

## ✅ Неделя 4: Feed стейты — ВЫПОЛНЕНО
...
2. ✅ Настроены переходы в `transitions.yaml`

---

## ✅ Неделя 5: Локализация — ВЫПОЛНЕНО

**Цель:** Полный перенос на модульную систему i18n.

**Выполнено:**

1. ✅ Создана модульная структура `i18n/`:
   - `i18n/__init__.py` — экспорты
   - `i18n/loader.py` — класс I18n с загрузкой YAML
   - `i18n/checker.py` — CLI для проверки переводов
   - `i18n/schema.yaml` — мастер-файл (ru + en, 236 ключей)
   - `i18n/translations/es.yaml` — испанский перевод

2. ✅ Все 3 языка переведены на 100%:
   - Русский: 236/236 ключей
   - Английский: 236/236 ключей
   - Испанский: 236/236 ключей

3. ✅ Генерация контента на выбранном языке (Claude API)

4. ✅ Удалён старый `locales.py`

5. ✅ Созданы тесты `tests/i18n/`
...
**Цель:** Реализовать диагностику уровня пользователя.
...
1. Удалить `engines/` (старый код марафона и ленты)

2. Обновить документацию

3. Полное тестирование всех сценариев

4. Установить `use_state_machine: true` по умолчанию
...
**Цель:** Добавить новые программы обучения.

**Мастерские:**

1. `workshop.exocortex.*` — Экзокортекс (настройка PKM)
   - audit.py — аудит текущего состояния
   - tools.py — выбор инструментов
   - setup.py — настройка
   - practice.py — практика

2. `workshop.fpfkids.*` — FPFkids (обучение детей)
   - goals.py — определение целей
   - topics.py — выбор тем
   - plan.py — составление плана
   - session.py — проведение занятия

3. `workshop.practice.*` — Задачник
   - topic_select.py — выбор темы
   - problem.py — показ задачи
   - solution.py — проверка решения
...
**Цель:** Создать Telegram Mini App для расширенного интерфейса.
...
|---------|-----|----------|
| Имя | Input | Редактирование имени |
| Сфера деятельности | Input | Профессия/занятие |
| Язык | Select | ru / en / es |
| Уровень сложности | Slider (1-3) | Сложность вопросов |
| Время обучения | Select | 5 / 10 / 15 / 20 мин |
| Часовой пояс | Select | Для напоминаний |

### 2. Выбор тем на неделю
| Элемент | Тип | Описание |
|---------|-----|----------|
| Категории | Tabs | Системное мышление, Личное развитие, ... |
| Темы | Cards с чекбоксами | Визуальный выбор (вместо inline-кнопок) |
| Прогресс | Progress bar | Сколько тем выбрано |
| Кнопка "Готово" | Button | Подтверждение выбора |

### 3. Статистика и прогресс
| Элемент | Тип | Описание |
|---------|-----|----------|
| Streak | Counter + fire | Дней подряд |
| Марафон | Progress bar | День X из 14 |
| Лента | Chart | Дайджесты по неделям |
| Темы | List | Пройденные темы с датами |
| Достижения | Badges | Геймификация |

**Технический стек:**
- Frontend: React + Telegram Mini Apps SDK
- Backend: FastAPI endpoint в боте
- Авторизация: Telegram WebApp initData

**Интеграция с ботом:**
- Mini App вызывается через кнопку в меню
- Данные синхронизируются с БД бота
- После изменений — уведомление в чат
...
## Архитектура
1. **State Machine** — один файл = один стейт
...
4. **Консультация едина:** Глобальный процесс `common.consultation` маршрутизирует вопросы
5. **Именование:** Стейты называются по **процессам** (consultation), а не по агентам

## Статус миграции

| Этап | Недели | Статус |
|------|--------|--------|
| Инфраструктура | 1 | ✅ Выполнено |
| Marathon стейты | 2 | ✅ Выполнено |
| Консультация | 3 | ✅ Выполнено |
| Feed стейты | 4 | ✅ Выполнено |
| Локализация i18n | 5 | ✅ Выполнено |
| Утилиты | 6 | 📋 В плане |
| Тест ступени | 7 | 📋 В плане |
| Очистка | 8 | 📋 В плане |
| Новые мастерские | 9 | 📋 В плане |
| Mini App | 10 | 📋 В плане |

## Текущие возможности
- ✅ State Machine работает с feature flag
- ✅ Марафон: lesson → question → bonus → task
- ✅ Лента: topics → digest
- ✅ Консультация: глобальный вызов через `?`
- ✅ i18n: 3 языка (ru/en/es), 236 ключей, 100% покрытие
- ✅ Генерация контента на выбранном языке

## Будущее
- **DigitalTwin:** Готовность через абстракцию StateStorage
- **Ory Identity:** Проверка прав при переходах
- **Mini App:** Расширенный UI для профиля, тем и статистики
